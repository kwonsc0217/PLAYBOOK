/** Confluence 위키 파싱 — Vercel/serverless용. lib/claude 호출로 Design 어셋 이름 정규화 */
import { callClaudeForText } from './claude.js';
...
// 추가 참고 페이지 (AI 컨텍스트에만 포함, 어셋/FAQ 파싱 안 함)
const CONFLUENCE_EXTRA_PAGES = (process.env.CONFLUENCE_EXTRA_PAGES || '')
...
async function normalizeAssetNamesWithAI(names) {
  if (!Array.isArray(names) || names.length === 0 || !ANTHROPIC_API_KEY) return null;
...
예: ["Logo", "KEYART", "SNS Images"]`;
  try {
    const raw = await callClaudeForText(prompt, 300);
...
// Confluence 위키 캐시. parts[partId] = { content, assets }, faq = 전용 FAQ 페이지에서 파싱, extraContext = 추가 참고 페이지 텍스트
let wikiCache = { parts: {}, faq: [], extraContext: '', ts: 0 };
...
function extractImageUrlsFromHtml(html) {
  ...
  while ((m = aHref.exec(html)) !== null) {
    ...
    if (/\.(mp4|webm|mov|ogv|avi)(\?|$)/i.test(u) || /youtube\.com\/(watch|shorts)|youtu\.be\//i.test(u)) urls.push(u);
  }
...
async function fetchWikiContent() {
  ...
  try {
    ...
    // 추가 참고 페이지 (어셋/FAQ 파싱 없이 텍스트만)
    let extraParts = [];
    for (const extraId of CONFLUENCE_EXTRA_PAGES) {
      try {
        const { html: extraHtml } = await fetchConfluencePage(extraId);
        const extraText = htmlToPlainText(extraHtml).trim();
        if (extraText) extraParts.push(extraText);
      } catch (e) {
        console.error('[Confluence 추가 페이지]', extraId, e.message || e);
      }
    }
    next.extraContext = extraParts.join('\n\n---\n\n');
    if (next.extraContext) console.log('[Confluence 위키] 추가 참고 페이지', CONFLUENCE_EXTRA_PAGES.length, '건 로드');
...
function getWikiContent(partId = 'design') {
  const part = wikiCache.parts && wikiCache.parts[partId];
  const partContent = (part && part.content) || '';
  const extra = wikiCache.extraContext || '';
  if (!extra) return partContent;
  return partContent + '\n\n---\n[추가 참고 자료]\n' + extra;
}
