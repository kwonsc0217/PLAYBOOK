...
const WIKI_CACHE_TTL_MS = Math.max(1, parseInt(process.env.WIKI_CACHE_TTL || '10', 10)) * 1000;
...
function parseRequiredAssetsFromHtml(html) {
  ...
  while ((tableMatch = tableRegex.exec(html)) !== null) {
    ...
    const col = getColumnIndices(headerCells);
    const out = [];
    let lastCategory = '';
    for (let i = headerRowIndex + 1; i < grid.length; i++) {
      ...
      const stepsStr = col.steps >= 0 ? (cells[col.steps] ?? '').trim() : '';
